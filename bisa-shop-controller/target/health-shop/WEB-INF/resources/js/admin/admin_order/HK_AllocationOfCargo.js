        ...
        $(".cargo_v1_box_add").click(function() {
            if (sel_val == "" || sel_val == null) {
                ...
                if (cargo_ipt_val == null || cargo_ipt_val == "" || typeof(cargo_ipt_val) == "undefined") {
                    ...
                    //再此处判断一下是否数据库中有,如果是出库了，则不能添加，反之就出库
                    cargo_ipt_val.replace('+', '%2B');
                     $.ajax({
                        url: path + "auth/admin/findInventory",
                        type: "get",
                        dataType: "text",
                        async: false,
                        data: {
                            "cargo_ipt_val": cargo_ipt_val,
                        },
                        success: function(result) {
                         if(result==0){
                            layer.alert('这个产品已经出库了，不能再添加', {
                                icon: 2,
                                title: '这个产品已经出库了，不能再添加',
                             });
                         }else{
                        ...
                        error: function() {
                            layer.alert('查不到这个库存,请联系管理员', {
                                icon: 2,
                                title: '查不到这个库存,请联系管理员',
                             });
                        }
                    });
                ...
                layer.confirm('您确定要配货么？', {
                    ...
                    for(var i=0;i<server_data.treeroot.length;i++){
                        ...
                        if(server_data.treeroot[i].state==1){
                            ...
                            json2.type=2;
                            json2.Device_id="";
                            json2.user_guid=uuid;
                            json2.order_no=objnum;
                            json4.push(json2)
                        } 
                    }
                    for(var i=0;i<server_data.treeroot.length;i++){
                        if(server_data.treeroot[i].state==0){
                            for(var a=0;a<server_data.treeroot[i].des.length;a++){
                                 ...
                                 json3.type=1;
                                 json3.count=1;
                                 json3.user_guid=uuid;
                                 json3.order_no=objnum;
                                 json4.push(json3);
                            }
                        } 
                    }
                    ...
                    $.ajax({
                        url: path + "auth/admin/adminInventorysubmit",
                        type: "POST",
                        dataType: "text",
                        async: false,
                        data: {
                        ...
                        success: function(result) {
                            if(result=="2"){
                                layer.confirm('请先去登录，登录后可以开始配货？', {
                                  btn: ['确定'] //按钮
                                }, function(){
                                  window.location.href=path+"/admin/login";
                                });
                            }else if(result=="3"){
                                layer.alert('订单的详情出错了,请联系管理员', {
                                    icon: 2,
                                    title: '订单的详情出错了,请联系管理员',
                                });
                            }else if(result=="4"){
                                layer.alert('配货出库错误,请联系管理员', {
                                    icon: 2,
                                    title: '配货出库错误,请联系管理员',
                                });
                            }else if(result=="5"){
                               layer.confirm('配货成功！是否跳转回配货页面', {
                                  btn: ['确定'] //按钮
                                }, function(){
                                  window.location.href=path+"auth/admin/admin_shippingclerk";
                                }); 
                            }
                        },
                        error: function() {
