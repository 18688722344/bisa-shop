    ...
    layui.use(['form', 'table', 'element', 'jquery'], function () {
        ...
        form.on('submit(check_success)', function (data) {
            ...
            layer.confirm('确认审核通过!', {icon: 3, title: '确认审核通过', btn: ['确认', '我再想想']}, function (index) {
                $.ajax({
                    url: 'admin/order/serviceCheckSuccess',
                    data: {
                        orderNO: order_no,
                        userId: user_id,
        ...
        form.on('submit(check_fail)', function (data) {
            ...
            layer.confirm('确认审核拒绝!', {icon: 3, title: '确认审核拒绝', btn: ['确认', '我再想想']}, function (index) {
                $.ajax({
                    url: 'admin/order/serviceCheckFail',
                    data: {
                        orderNO: order_no,
                        userId: user_id,
                        description: description,
        ...
        table.render({
            ...
            id: 'DeliverList',
            url: 'admin/order/serviceList',
            limit: 10,
            page: true,
            cols: [
                [ //标题栏
                    {type: 'numbers'},
                    {field: 'id', title: 'ID', width: 60, event: 'id', sort: true},
                    ...
                    {field: 'total_price', title: '商品总额', width: 100, sort: true},
                    {field: 'preferential_price', title: '优惠价格', width: 100, sort: true},
                    {field: 'post_price', title: '邮费', width: 100, sort: true},
                    {field: 'actual_payment', title: '实付款', width: 100, sort: true},
        ...
        table.on('tool(DLtest)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //待审核
            if (layEvent === 'check') {
                $(".user_id").val(data.user_id);//用户的id
