$(document).ready(function() {
    ...
    layui.use(['form', 'table', 'element'], function() {
        ...
        table.render({
            ...
            id: 'DeliverList',
            url: path+'auth/admin/service/adminsaleList_no_quote',
            height: 400, //容器高度
            cols: [
                [ //标题栏
                    ...
                    { field: 'orderno', title: '订单号', width: 240},
                    { field: 'sn', title: 'SN码', width: 160},
                    { field: 'product_title', title: '产品类型', width: 100},
                    { field: 'expiration_time', title: '申请时间', width: 160 },
        ...
        table.on('tool(DLtest)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') { //查看详情
                /*取data里面需要的值，跳转页面*/
                window.location.href=path+'auth/admin/service/selectSalesafter/'+data.salesafter_guid+'/'+data.user_guid;
            };
            if (layEvent === 'repair_record') { //维修记录
                ...
                var set_tim_out = setTimeout(function open_detael(){
                    ...
                    for (var i = 0; i < service_log_data.length; i++) {
                       ...
                       var new_sn="";
                        if(service_log_data[i].new_sn==""||service_log_data[i].new_sn==null||service_log_data[i].new_sn==undefined){
                            new_sn="无";
                        }else{
                           new_sn= service_log_data[i].new_sn;
                        }
                        var repair_txt="";
                        if(service_log_data[i].repair_txt==""||service_log_data[i].repair_txt==null||service_log_data[i].new_sn==undefined){
                            repair_txt="无";
                        }else{
                            repair_txt=service_log_data[i].repair_txt;
                        }
                        var money;
                        if(service_log_data[i].money==""||service_log_data[i].money==null||service_log_data[i].new_sn==undefined){
                            money="无";
                        }else{
                            money=service_log_data[i].money;
                        }
                            tr_list += "<tr>";
